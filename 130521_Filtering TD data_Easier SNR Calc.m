...
Min_thickness=50000; %pixel

Max_thickness=100000; %pixel

cd('D:\Users\TuanShu\130521\Run 2\');

qqq=1;
xxx=1;
yyy=1;
for j=2:2     %14 or 15: first peak, 11 or 12: rear peak
...
Data_New(1:Delay_first_peak)=Data_New(Delay_first_peak);
Data_New((length(Data_New)-Delay_first_peak):end)=Data_New((length(Data_New)-Delay_first_peak));

Data_New((length(Data_New)-40):end)=Data_New((length(Data_New)-40));
Data_New=abs(Data_New);
Window_ave=(1:Averaging)/Averaging;
Data_New=conv(Data_New,Window_ave,'same');
Data_New=Data_New/max(abs(Data_New));
...
q=1;
    ...
    for w=1:length(max_index_array_raw)
        Data_Temp=Data_New/max(abs(Data_New));
...
plot(thickness_array_1);
xlabel('Nth Scan');
ylabel('Measured Thickness (OPD, micron)');


plot(thickness_array_2);
xlabel('Nth Scan');
ylabel('Measured Thickness (OPD, micron)');
