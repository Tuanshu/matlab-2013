...
N_Shift=[1:50]';     27%Physically must be integer, 若不是integer那會算錯
...
if length(Thickness)==1 %if length(Thickness)>1 > Uniqueness test
    ...
    %xlim([4.65 4.8]);
        ...
        MSE_Normalized_1=((MSE_A_Record_1.^2+MSE_B_Record_1.^2+MSE_C_Record_1.^2)/min(MSE_A_Record_1.^2+MSE_B_Record_1.^2+MSE_C_Record_1.^2)).^0.5;
        MSE_Normalized_2=((MSE_B_Record_1.^2)/min(MSE_B_Record_1.^2)).^0.5;
        subplot(1,3,1)
        plot(N_Shift,MSE_A_Record/min(MSE_A_Record));
        xlabel('N');
        ylabel('MSE_A');
        ylim([0 100]);
        xlim([0 50]);
        ...
        subplot(1,3,2)
        plot(N_Shift,MSE_B_Record/min(MSE_B_Record));
        xlabel('N');
        ylabel('MSE_B');
        ylim([0 100]);
        xlim([0 50]);
        ...
        subplot(1,3,3)
        plot(N_Shift,MSE_C_Record/min(MSE_C_Record));
        xlabel('N');
        ylabel('MSE_C');
        ylim([0 100]);
        xlim([0 50]);
