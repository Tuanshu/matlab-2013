...
Sample_Path_Bscan='D:\Users\TuanShu\130129\BinnedSam\Bscan2\';
Reference_Path='D:\Users\TuanShu\130129\BinnedRef\';
Reference_Path_Bscan='D:\Users\TuanShu\130129\BinnedRef\\Bscan2\';
...
for qqq=1:1
    ...
    plot(Position_micron,Image_Stack_New(:,35,45));
    
    
    
    Image_Stack_New(1:1800,:,:)=0;
    Image_Stack_New(2800:end,:,:)=0;
    
    
    %Max_Value_of_The_Volume=max(abs(Image_Stack_New(:)));
    %Image_Stack_New_Normalized=Image_Stack_New./max(abs(Image_Stack_New(:)));
    Image_Stack_New_Abs=log10(abs(Image_Stack_New))/max(max(max(log10(abs(Image_Stack_New)))));
    if qqq==1
        cd(Sample_Path_Bscan);
        %dlmwrite('Max_Value_Sam.txt',Max_Value_of_The_Volume,'delimiter','\t','newline','pc');
        for rr=1:size(Image_Stack_New_Abs,3)
            Temp_Bscan(:,:)=Image_Stack_New_Abs(:,:,rr);
