...
Thickness=[4.57:0.0005:4.58]*1E-6;%[4.54:0.002:4.6].*1E-6;      %Goal Value (4.6890e-06)
n_should=1.6;

Uniqueness_Test_Subject='Refractive Index'; % Refractive Index, Extinction Coefficient, Thickness

if strcmp(Uniqueness_Test_Subject,'Refractive Index')
    Scanning_Parameter

Max_Number_of_Loop=3000;

Merit_A_OK=0;%0.0001;         %mean(delta_A./A)
Merit_B_OK=0;%0.0001;         %mean(delta_B./B)
Merit_C_OK=0;%0.01;         %mean(delta_C./C)

Wavelength_Center_for_Thickness_Weighting=540;
Wavelength_BW_for_Thickness_Weighting=5;

Uniqueness_Percent=1.1;

Only_Calc_The_First=0;

C_Reduction_Factor=1;
...
Wavelength_Considered_Min=539.75;          %nm
...
Cexp=unwrap(Cexp,[],1)/C_Reduction_Factor;
...
C = @ (n,Thickness_Now,Ratio_Lower2Upper_Now,Ratio_Upper2Reference_Now) unwrap(angle(1./r1(n).*(Ratio_Lower2Upper_Now.*(t1(n).*t1_r(n).*r2(n).*exp(1i*4*pi.*Frequency_Considered.*n.*Thickness_Now/c)))),[],1)/C_Reduction_Factor; %雖然沒有絕對的phase, 但是是不是應該只shift pi的整數?
...
Merit_C=99999999;
Current_Progress=0;
Merit_A_Record_2(1:length(Thickness))=0;
Merit_B_Record_2(1:length(Thickness))=0;
Merit_C_Record_2(1:length(Thickness))=0;
    ...
    for p=1:length(Thickness)
        ...
        for q=1:length(Ratio_Lower2Upper)
            for w=1:length(Ratio_Upper2Reference)
                    ...
                    Thickness_Now(1:length(Frequency_Considered),1:Active_Array_Size)=Thickness(p);
                    ...
                    while (Current_Loop<Max_Number_of_Loop) && ((Merit_A>Merit_A_OK)||(Merit_B>Merit_B_OK)||(Merit_C>Merit_C_OK))
                        ...
                        delta_C=Cexp-C_Temp;                        
                        %Merit_A=mean((delta_A./Aexp).^2).^0.5;
                        %Merit_B=mean((delta_B./Bexp).^2).^0.5;
                        %Merit_C=mean((delta_C./Cexp).^2).^0.5;
                        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                        ...
                        n=n+0.01*dn_real+0.6*1i*dn_imag;
        ...
        Merit_A_Record_2(p)=(sum((delta_A./Aexp).^2)/length(delta_A));
        Merit_B_Record_2(p)=(sum((delta_B./Bexp).^2)/length(delta_B));
        Merit_C_Record_2(p)=(sum((delta_C./Cexp).^2)/length(delta_C));
        %fprintf('Merit_A=%e, Merit_B=%e, Merit_C=%f\n',Merit_A,Merit_B,Merit_C);
    end
end
plot(Frequency_Considered,Cexp,Frequency_Considered,C_Temp)

subplot(3,1,1)
plot(Thickness*1E6,Merit_A_Record_2);
xlabel('Thickness (micron)');
ylabel('MSE_A^2');
%xlim([4.5 4.7]);

subplot(3,1,2)
plot(Thickness*1E6,Merit_B_Record_2);
xlabel('Thickness (micron)');
ylabel('MSE_B^2');
%xlim([4.5 4.7]);

subplot(3,1,3)
plot(Thickness*1E6,Merit_C_Record_2);
xlabel('Thickness (micron)');
ylabel('MSE_C^2');
%xlim([4.5 4.7]);
...
MSE_Normalized=((Merit_A_Record_2+Merit_B_Record_2+Merit_C_Record_2)/min(Merit_A_Record_2+Merit_B_Record_2+Merit_C_Record_2)).^0.5;

Uniqueness_Range=abs(Thickness(find(MSE_Normalized<1.1,1,'first'))-Thickness(find(MSE_Normalized<1.1,1,'last')));
