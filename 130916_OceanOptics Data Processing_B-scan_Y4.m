...
cd('D:\Users\TuanShu\130916_5.5ms\');
%% Data Loading
...
Signal_Bscan_Envelope(:,jj)=abs(Signal(1:size(Signal_Bscan_Envelope,1)));
if jj==1
...
%%
plot(Position_micron,Signal_Bscan_Envelope(:,32));
xlabel('OPD (micron)');
...
plot(Position_micron_Cut,Signal_Bscan_Envelope_Cut(:,1:30));
xlabel('Optical Path Difference (micron)');
ylabel('Normalized Interference Intensity (a.u.)');

plot(Position_micron_Cut(max_index(1:30)),max_value(1:30)/max(max(max_value(1:30))));
xlabel('Optical Path Difference (micron)');
ylabel('Normalized Interference Intensity (a.u.)');
%xlabel('Lateral Position (micron)');
%ylabel('Optical Path Difference (micron)');
