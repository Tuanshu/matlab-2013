...
incident=2;     %1: sub, 2: air
N_Shift=[-60:5:-40]';     %Physically must be integer, 若不是integer那會算錯
...
Wavelength_BW_for_Thickness_Weighting=20;
...
elseif incident == 2    
    Sample_Path='D:\Users\TuanShu\130522\Y_air incident\sample\';
    Reference_Path='D:\Users\TuanShu\130522\Y_air incident\glass\';
    Spectroscopy_Path='D:\Users\TuanShu\1208030331_5micron 1.jws.txt';
end
...
Ratio_Upper2Reference=[0.9]; %if the front interface has larger interference eff than referential glass, Ratio_Upper2Reference>1
Thickness=[4.5650]'*1E-6;      %Initial Value
cd('D:\Users\TuanShu\');
n_should=dlmread('n.txt')+1i*dlmread('k.txt');
Ratio_Lower2Upper=[1.04];     %if the rear interface has larger interference eff than front interface, Ratio_Lower2Upper>1

%%%
DC_cutoff=1;   %works for all lateral position
Least_Separation=2;
Window_Size_Right_1=9;  %use for the left side of 1st interface and right side of second interface
Window_Size_Left_1=10;  %use for the left side of 1st interface and right side of second interface
Window_Size_Right_2=6;  %use for the left side of 1st interface and right side of second interface
...
Wavelength_Considered_Min=530; %510         %nm
Wavelength_Considered_Max=560;  %580
...
Separation_index=find(Position_micron>47,1,'first');
...
plot(Position_micron(Position_micron<60),Signal_Sample_1(Position_micron<60),Position_micron(Position_micron<60),Signal_Sample_2(Position_micron<60),Position_micron(Position_micron<60),Signal_Reference(Position_micron<60));
...
Cunwrap=Cunwrap-Cunwrap_intercept;%-C_intercept;
...
if length(n_should)==1
    ...
    n(1:length(Frequency_Considered),1:Active_Array_Size)=repmat(n_should,1,Active_Array_Size);
end
        ...
        for q=1:length(Ratio_Lower2Upper)
            for w=1:length(Ratio_Upper2Reference)
                    ...
                    while (Current_Loop<Max_Number_of_Loop) && ((Merit_A>Merit_A_OK)||(Merit_B>Merit_B_OK)||(Merit_C>Merit_C_OK))
                        ...
                        if length(Thickness)==1 %if length(Thickness)>1 > Uniqueness test
                            Thickness_Now=Thickness_Now+0.1*mean(dthickness);   %How if I make the thickness a array during the iteration?
                            %Thickness_Now=Thickness_Now+0.5*sum(Weighting_Function_for_Thickness.*dthickness);
                        end
                        ...
                        MSE_A_Record_Temp=(sum((delta_A./Aexp).^2)/length(delta_A)).^0.5;
                        MSE_B_Record_Temp=(sum((delta_B./Bexp).^2)/length(delta_B)).^0.5;
                        MSE_C_Record_Temp=(sum((delta_C./Cexp).^2)/length(delta_C)).^0.5;
                        Merit=(MSE_A_Record_Temp.^2+MSE_B_Record_Temp.^2+MSE_C_Record_Temp.^2).^0.5;
...
if length(Thickness)==1 %if length(Thickness)>1 > Uniqueness test
    if length(N_Shift)==1
        ...
        fprintf('Thickness_Best=%d, Ratio_Lower2Upper_Best=%f, Ratio_Upper2Reference_Best=%f\n',Thickness_Best,Ratio_Lower2Upper_Best,Ratio_Upper2Reference_Best);
    else
        mean(Thickness_Now)
        subplot(4,1,1)
...
fprintf('N_Shift=%d, MSE_A=%e, MSE_B=%e, MSE_C=%e\n',N_Shift(index_show),MSE_A_Record(index_show),MSE_B_Record(index_show),MSE_C_Record(index_show));
