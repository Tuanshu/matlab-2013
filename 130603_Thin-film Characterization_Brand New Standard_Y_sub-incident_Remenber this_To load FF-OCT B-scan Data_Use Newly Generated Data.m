...
N_Shift=[27]';     27%Physically must be integer, 若不是integer那會算錯
...
Sample_Path='D:\Users\TuanShu\130117\130117_Sam\BscanNew\';
Reference_Path='D:\Users\TuanShu\130117\130117_Ref\BscanNew\';
...
Ratio_Upper2Reference=1.;%0.835; %if the front interface has larger interference eff than referential glass, Ratio_Upper2Reference>1
Thickness=[4.4]'*1E-6;      %Initial Value
if load_nk ==1
    n_should=dlmread('n.txt')+1i*dlmread('k.txt');
elseif load_nk ==0
    n_should=1.6;
end
Ratio_Lower2Upper=0.62;     %if the rear interface has larger interference eff than front interface, Ratio_Lower2Upper>1
...
Window_Size_Left_1=3;  %use for the left side of 1st interface and right side of second interface
Window_Size_Right_2=4.3;  %use for the left side of 1st interface and right side of second interface
Window_Size_Left_2=4;  %use for the left side of 1st interface and right side of second interface
...
Wavelength_Considered_Max=580;  %580
...
Considered_Lateral_Index_Range=1;
Considered_Axial_Index_Range=1:3000;
Number_of_Axial_point_After_Exapnding=100000;

Select_Position=[35 45];


N_f=Number_of_Axial_point_After_Exapnding;
N_t=Number_of_Axial_point_After_Exapnding;
...
Signal_Sam=Frame_Temp(:,Select_Position(1));
cd(Reference_Path);
Frame_Temp=dlmread(sprintf('Bscan_Ref_%i.txt',Select_Position(2))); 
Signal_Ref=Frame_Temp(:,Select_Position(1));
...
[maxvalue_Ref maxindex_Ref]=max(Signal_Ref,[],1);
...
Position_micron=Position_micron;

for p=1:length(Considered_Lateral_Index_Range)
    Signal_Sam((length(Considered_Axial_Index_Range)+1):Number_of_Axial_point_After_Exapnding,p)=0;
    Signal_Ref((length(Considered_Axial_Index_Range)+1):Number_of_Axial_point_After_Exapnding,Considered_Lateral_Index_Range(p))=0;
end
ifft_Signal_Sam=ifft(Signal_Sam);
ifft_Signal_Ref=ifft(Signal_Ref);
ifft_Signal_Sam(1:2000)=0;
ifft_Signal_Sam(4500:end)=0;
ifft_Signal_Ref(1:2000)=0;
ifft_Signal_Ref(4500:end)=0;
Signal_Sample=(fft(ifft_Signal_Sam));
Signal_Reference=(fft(ifft_Signal_Ref));
plot(1:length(ifft_Signal_Sam),ifft_Signal_Sam,1:length(ifft_Signal_Sam),ifft_Signal_Ref);
plot(Position_micron,Signal_Sample,Position_micron,Signal_Reference);

c=3E8;

Signal_Sample(isnan(Signal_Sample))=0;
Signal_Reference(isnan(Signal_Reference))=0;
Spectrum_Sam=ifft(real(Signal_Sample),[],1);
Spectrum_Ref=ifft(real(Signal_Reference),[],1);
Spectrum_Sam((fix(size(Spectrum_Sam,1)/2)+1):end,:)=0;
Spectrum_Ref((fix(size(Spectrum_Ref,1)/2)+1):end,:)=0;
...
Spectrum_Sample_1((fix(size(Spectrum_Sample_1,1)/2)+1):end,:)=0;

Spectrum_Sample_2=ifft(Signal_Sample_2,[],1);
Spectrum_Sample_2=2*Spectrum_Sample_2(1:N_f,:);
Spectrum_Sample_2((fix(size(Spectrum_Sample_2,1)/2)+1):end,:)=0;

Spectrum_Reference=ifft(Signal_Reference,[],1);
Spectrum_Reference=2*Spectrum_Reference(1:N_f,:);                
Spectrum_Reference((fix(size(Spectrum_Reference,1)/2)+1):end,:)=0;
        ...
        for q=1:length(Ratio_Lower2Upper)
            for w=1:length(Ratio_Upper2Reference)
                    ...
                    while (Current_Loop<Max_Number_of_Loop) && ((Merit_A>Merit_A_OK)||(Merit_B>Merit_B_OK)||(Merit_C>Merit_C_OK))
                        ...
                        n=n+0.01*dn_real+0.01*1i*dn_imag;
