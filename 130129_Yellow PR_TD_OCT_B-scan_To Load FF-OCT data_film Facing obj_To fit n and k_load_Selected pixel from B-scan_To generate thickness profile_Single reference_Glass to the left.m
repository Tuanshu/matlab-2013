...
Bscan_index_array=1:61;

Reference_Position=[30 30];

Lateral_Pixel_Size=0.36*8; %micron
...
Sample_Path='D:\Users\TuanShu\130129\BinnedSam\Bscan\';
Reference_Path='D:\Users\TuanShu\130129\BinnedRef\\Bscan\';
...
Considered_Lateral_Index_Range=[1:80];
...
cd(Reference_Path);
Frame_Temp=dlmread(sprintf('Bscan_Ref_%i.txt',Reference_Position(2))); 
Signal_Ref=Frame_Temp(:,Reference_Position(1));
...
%for p=1:length(Considered_Lateral_Index_Range) % 因為現在的B-scan data在前一個檔案就filter過了, 所以現在可以用簡單的方法
%    Signal_Sam((length(Considered_Axial_Index_Range)+1):Number_of_Axial_point_After_Exapnding,p)=Signal_Sam(length(Considered_Axial_Index_Range),p);
%    %Signal_Ref((length(Considered_Axial_Index_Range)+1):Number_of_Axial_point_After_Exapnding,Considered_Lateral_Index_Range(p))=Signal_Ref(length(Considered_Axial_Index_Range),Considered_Lateral_Index_Range(p));
%end
...
cd(Sample_Path);
for bscanindex=1:length(Bscan_index_array)

clear Thickness_Profile

Frame_Temp=dlmread(sprintf('Bscan_Sam_%i.txt',Bscan_index_array(bscanindex))); 
Signal_Sam=Frame_Temp(:,:);

Signal_Sam=[Signal_Ref Signal_Sam];                                         %%%%%%%%%%%%%%%%%%%%%%%%% NOTE!!!!!!! 暫時這樣寫, 到時候FF-OCT可能會不太一樣
...
[maxvalue maxindex]=max(Signal_Sam,[],1);

Shift_Amount=1500-maxindex(1);  %乾脆直接用第一行來shift, circshift吃1d array好像會怪怪

Signal_Sam=circshift(Signal_Sam,Shift_Amount);      %他真的有每行shift不同的量嗎? 好像其實沒有, 不然照現在的寫法, reference應該也會被set到20 (可是沒有)
Signal_Sam((length(Considered_Axial_Index_Range)+1):Number_of_Axial_point_After_Exapnding,:)=0;

Signal_Sam(isnan(Signal_Sam))=0;
...
%Signal_Ref_Filtered=ifft(Spectrum_Ref,[],1);
...
Safe_Reference_Index=round((First_With_Second_Index+length(Last_No_Second_Index))/2);
...
if Only_Calc_The_First == 1
    ...
    Active_Array_Size=length(Considered_Lateral_Index_Range(First_With_Second_Index:end));
...
%plot(Position_micron(Position_micron<50),Signal_Sample_1(Position_micron<50,3),Position_micron(Position_micron<50),Signal_Sample_2(Position_micron<50,3),Position_micron(Position_micron<50),Signal_Sample_1(Position_micron<50,1));
...
Signal_Reference=Signal_Sample_1(:,Safe_Reference_Index);
Spectrum_Reference=Spectrum_Sample_1(:,Safe_Reference_Index);

%plot(Wavelength_micron(Wavelength_micron<1),abs(Spectrum_Sample_1(Wavelength_micron<1,20)),Wavelength_micron(Wavelength_micron<1),abs(Spectrum_Sample_2(Wavelength_micron<1,20)),Wavelength_micron(Wavelength_micron<1),abs(Spectrum_Sample_1(Wavelength_micron<1,1)));           %若是頻譜解析度太差, 是不是也會因為Signal 
%xlabel('Wavelength (micron)');
%ylabel('Normalized Spectrum');
%legend('Front Interface','Rear Interface','Reference Interface');

%
Spectrum_Sample_1=Spectrum_Sample_1(:,(size(Spectrum_Sample_1,2)-Active_Array_Size+1):end);
Spectrum_Sample_2=Spectrum_Sample_2(:,(size(Spectrum_Sample_1,2)-Active_Array_Size+1):end);   %!!!!!!!!!! 因為接下來就要算n k 了, 把沒有兩個介面的放進來情況會很怪
...
OPD_Map(:,bscanindex)=Thickness_Profile;
...
Thickness_Now(1:length(Frequency_Considered),1:Active_Array_Size)=Thickness*Thickness_Profile/6.6;
...
if index_calculation==1   
        ...
        for q=1:length(Ratio_Lower2Upper)
            for w=1:length(Ratio_Upper2Reference)
                    ...
                    while (Current_Loop<Number_of_Loop)
                        ...
                        n(isnan(n))=n_should;
                        n(isinf(n))=n_should;
                        if max(max(isnan(Thickness_Now)>0))||max(max(isinf(Thickness_Now)))
                            %Thickness_Now(isnan(Thickness_Now))=Thickness*Thickness_Profile(isnan(Thickness_Now))/max(max(Thickness_Profile));
                            Thickness_Now(isnan(Thickness_Now))=Thickness*Thickness_Profile(isnan(Thickness_Now))/6.6;
                            %Thickness_Now(isinf(Thickness_Now))=Thickness*Thickness_Profile(isinf(Thickness_Now))/max(max(Thickness_Profile));
                            Thickness_Now(isinf(Thickness_Now))=Thickness*Thickness_Profile(isinf(Thickness_Now))/6.6;
                        end
                        if max(max(abs(Thickness_Now)>6*1E-6))
                            %Thickness_Now(abs(Thickness_Now)>6*1E-6)=Thickness*Thickness_Profile(abs(Thickness_Now)>6*1E-6)/max(max(Thickness_Profile));
                            Thickness_Now(abs(Thickness_Now)>6*1E-6)=Thickness*Thickness_Profile(abs(Thickness_Now)>6*1E-6)/6.6;
                        end
                        %if Current_Loop>70
                        %    n_Record(:,qqqq)=n;
 ...
 Thickness_Best(Thickness_Best>6E-6)=0;
 Thickness_Best(1:Last_No_Second_Index)=0;
 Thickness_Map(:,bscanindex)=Thickness_Best;
 disp(sprintf('Bscan_index=%i\n',Bscan_index_array(bscanindex)));
end
...
%ylabel('k');
% 175.6800*  230.4000
%imagesc(Thickness_Map*1E6,'xdata',Lateral_Pixel_Size:Lateral_Pixel_Size:Lateral_Pixel_Size*size(Thickness_Map,2),'ydata',Lateral_Pixel_Size:Lateral_Pixel_Size:Lateral_Pixel_Size*size(Thickness_Map,1));
imagesc(Thickness_Map*1E6);
xlabel('(micron)');
ylabel('(micron)');
colorbar
axis equal
imagesc(OPD_Map,'xdata',Lateral_Pixel_Size:Lateral_Pixel_Size:Lateral_Pixel_Size*size(OPD_Map,2),'ydata',Lateral_Pixel_Size:Lateral_Pixel_Size:Lateral_Pixel_Size*size(OPD_Map,1));
xlabel('(micron)');
ylabel('(micron)');
colorbar
axis equal
...
subplot(3,1,1)
plot(FFOCT_Wavelength_micron_Considered(abs(FFOCT_Wavelength_micron_Considered-0.54)<0.03),FFOCT_n(abs(FFOCT_Wavelength_micron_Considered-0.54)<0.03),SDOCT_Wavelength_micron(abs(SDOCT_Wavelength_micron-0.54)<0.03),SDOCT_n(abs(SDOCT_Wavelength_micron-0.54)<0.03));
xlabel('Wavelength (micron)');
ylabel('n');

subplot(3,1,2)
plot(FFOCT_Wavelength_micron_Considered(abs(FFOCT_Wavelength_micron_Considered-0.54)<0.03),FFOCT_k(abs(FFOCT_Wavelength_micron_Considered-0.54)<0.03),SDOCT_Wavelength_micron(abs(SDOCT_Wavelength_micron-0.54)<0.03),SDOCT_k(abs(SDOCT_Wavelength_micron-0.54)<0.03));
xlabel('Wavelength (micron)');
ylabel('k');

subplot(3,1,3)
plot(FFOCT_Wavelength_micron_Considered(abs(FFOCT_Wavelength_micron_Considered-0.54)<0.03),FFOCT_T(abs(FFOCT_Wavelength_micron_Considered-0.54)<0.03),SDOCT_Wavelength_micron(abs(SDOCT_Wavelength_micron-0.54)<0.03),SDOCT_T(abs(SDOCT_Wavelength_micron-0.54)<0.03));
xlabel('Wavelength (micron)');
ylabel('T');

dlmwrite('Thickness_Map.txt',(Thickness_Map*1E6)','delimiter','\t','newline','pc');
