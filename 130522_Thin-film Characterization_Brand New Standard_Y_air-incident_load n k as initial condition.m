...
N_Shift=[-60]';     %Physically must be integer, 若不是integer那會算錯
Max_Number_of_Loop=6000;
...
Ratio_Upper2Reference=[0.8:0.02:1]; %if the front interface has larger interference eff than referential glass, Ratio_Upper2Reference>1
Thickness=[4.5]'*1E-6;      %Initial Value
cd('D:\Users\TuanShu\');
n_should=dlmread('n.txt')+1i*dlmread('k.txt');
Ratio_Lower2Upper=[0.6:0.02:0.9];     %if the rear interface has larger interference eff than front interface, Ratio_Lower2Upper>1
...
Window_Size_Left_2=2.1;  %use for the left side of 1st interface and right side of second interface
        ...
        for q=1:length(Ratio_Lower2Upper)
            for w=1:length(Ratio_Upper2Reference)
                    ...
                    while (Current_Loop<Max_Number_of_Loop) && ((Merit_A>Merit_A_OK)||(Merit_B>Merit_B_OK)||(Merit_C>Merit_C_OK))
                        ...
                        n=n+0.1*dn_real+0.01*1i*dn_imag;  
