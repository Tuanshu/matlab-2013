...
Thickness=[4.2:0.01:4.8]*1E-6;      %Initial Value
n_should=1.6;
...
C = @ (n,Thickness_Now,Ratio_Lower2Upper_Now,Ratio_Upper2Reference_Now) unwrap(angle(1./r1(n).*(Ratio_Lower2Upper_Now.*(t1(n).*t1_r(n).*r2(n).*exp(1i*4*pi.*(Frequency_Considered+df).*n.*Thickness_Now/c)))),[],1);%-unwrap(angle(1./r1(n).*(Ratio_Lower2Upper_Now.*(t1(n).*t1_r(n).*r2(n).*exp(1i*4*pi.*Frequency_Considered.*n.*Thickness_Now/c)))),[],1); %雖然沒有絕對的phase, 但是是不是應該只shift pi的整數?
        ...
        for q=1:length(Ratio_Lower2Upper)
            for w=1:length(Ratio_Upper2Reference)
                    ...
                    while (Current_Loop<Max_Number_of_Loop) && ((Merit_A>Merit_A_OK)||(Merit_B>Merit_B_OK)||(Merit_C>Merit_C_OK))
                        ...
                        C_Temp=diff(C(n,Thickness_Now,Ratio_Lower2Upper_Now,Ratio_Upper2Reference_Now));
