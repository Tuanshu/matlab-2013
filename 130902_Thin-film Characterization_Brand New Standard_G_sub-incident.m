...
N_Shift=[16]';     %-31%Physically must be integer, 若不是integer那會算錯
...
MSE_C_OK=0.0001;         %mean(delta_C./C)
MSE_total_OK=0.005;         %mean(delta_C./C)
CE_total_OK=0.0001;
...
Sample_Path='D:\Users\TuanShu\130509\130509_sub_G_sam.txt';
Reference_Path='D:\Users\TuanShu\130509\130509_sub_G_glass.txt';
Spectroscopy_Path='D:\Users\TuanShu\TEST\111127 - 3ms 100ave 3mm v0.01 Green - 3\111118_Green 5-1.jws.txt';
...
Ratio_Upper2Reference=0.8; %if the front interface has larger interference eff than referential glass, Ratio_Upper2Reference>1
Thickness=[2.72]'*1E-6;      %Initial Value
n_should=1.6;
...
plot(Wavelength_micron(Wavelength_micron<1),Spectrum_Sample(Wavelength_micron<1),Wavelength_micron(Wavelength_micron<1),Spectrum_Reference(Wavelength_micron<1));
    ...
    for p=1:length(Thickness)
        ...
        for q=1:length(Ratio_Lower2Upper)
            for w=1:length(Ratio_Upper2Reference)
                    ...
                    CE_total=999999999;
                    qqqqqq=1;
                    while (Current_Loop<Max_Number_of_Loop) && (CE_total>CE_total_OK)
                        ...
                        MSE_total_rec(qqqqqq)=MSE_total;
                        qqqqqq=qqqqqq+1;
                        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                        ...
                        CE_total=((sum((dn_real./real(n)).^2)/length(n))+(sum((dn_imag./imag(n)).^2)/length(n))+((sum((dthickness)./Thickness).^2)/length(dthickness))).^0.5;
