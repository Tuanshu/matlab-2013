...
Thickness=[4:0.1:5]*1E-6;%[4.54:0.002:4.6].*1E-6;      %Goal Value (4.6890e-06)
n_should=1.6;
...
Wavelength_Considered_Min=539;          %nm
Wavelength_Considered_Max=541;
...
Merit_C=99999999;
Current_Progress=0;
Merit_A_Record(1:length(Thickness))=0;
Merit_B_Record(1:length(Thickness))=0;
Merit_C_Record(1:length(Thickness))=0;
...
if index_calculation==1   
    ...
    for p=1:length(Thickness)
        ...
        Merit_A_Record(p)=(sum((delta_A./Aexp).^2)/length(delta_A))^0.5;
        Merit_B_Record(p)=(sum((delta_B./Bexp).^2)/length(delta_B))^0.5;
        Merit_C_Record(p)=(sum((delta_C./Cexp).^2)/length(delta_C))^0.5;
...
subplot(3,1,1)
plot(Thickness*1E6,Merit_A_Record);
xlabel('Thickness (micron)');
ylabel('MSE_A');

subplot(3,1,2)
plot(Thickness*1E6,Merit_B_Record);
xlabel('Thickness (micron)');
ylabel('MSE_B');

subplot(3,1,3)
plot(Thickness*1E6,Merit_C_Record);
xlabel('Thickness (micron)');
ylabel('MSE_C');
