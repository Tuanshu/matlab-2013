...
N_Shift=[27]';     %Physically must be integer, 若不是integer那會算錯
Max_Number_of_Loop=3000;

n_UR_calc=1;                %1 for n, 2 for k
n_delta_array=-0.0001:0.00001:0.0001;
Scan_Pixel=50;
...
Merit_C_OK=0.0001;         %mean(delta_C./C)
...
Sample_Path='D:\Users\TuanShu\130512\Y2\sam1\';
Reference_Path='D:\Users\TuanShu\130512\Y2\glass1\';
...
Ratio_Upper2Reference=0.835;%0.835; %if the front interface has larger interference eff than referential glass, Ratio_Upper2Reference>1
Thickness=[4.4485]'*1E-6;      %Initial Value
if load_nk ==1
...
%%%
DC_cutoff=1;   %works for all lateral position
Least_Separation=2;
Window_Size_Right_1=4;  %use for the left side of 1st interface and right side of second interface
Window_Size_Left_1=5;  %use for the left side of 1st interface and right side of second interface
Window_Size_Right_2=5;  %use for the left side of 1st interface and right side of second interface
Window_Size_Left_2=4.1;  %use for the left side of 1st interface and right side of second interface
...
plot(Position_micron(Position_micron<55),Signal_Sample_1(Position_micron<55),Position_micron(Position_micron<55),Signal_Sample_2(Position_micron<55),Position_micron(Position_micron<55),Signal_Reference(Position_micron<55));
...
if length(N_Shift)>1 %if length(Thickness)>1 > Uniqueness test
    MSE_A_Record(1:length(N_Shift))=0;
    MSE_B_Record(1:length(N_Shift))=0;
    MSE_C_Record(1:length(N_Shift))=0;
elseif n_UR_calc>0
    MSE_A_Record_Wavelength(1:length(Wavelength_micron_Considered),1:length(n_delta_array))=0;
    MSE_B_Record_Wavelength(1:length(Wavelength_micron_Considered),1:length(n_delta_array))=0;
    MSE_C_Record_Wavelength(1:length(Wavelength_micron_Considered),1:length(n_delta_array))=0;
else
...
if index_calculation==1   
    ...
    for u=1:length(n_delta_array)
        
        if length(n_should)==1
            n(:,1:Active_Array_Size)=n_should;
        elseif n_UR_calc==1
            %n(:,1:Active_Array_Size)=repmat(n_should,1,Active_Array_Size)+n_delta_array(u);
            n(:,1:Active_Array_Size)=repmat(n_should,1,Active_Array_Size);
            n(Scan_Pixel,1:Active_Array_Size)=n(Scan_Pixel,1:Active_Array_Size)+n_delta_array(u);
        elseif n_UR_calc==2
            %n(:,1:Active_Array_Size)=repmat(n_should,1,Active_Array_Size)+1i*n_delta_array(u);
            n(:,1:Active_Array_Size)=repmat(n_should,1,Active_Array_Size);
            n(Scan_Pixel,1:Active_Array_Size)=n(Scan_Pixel,1:Active_Array_Size)+1i*n_delta_array(u);
        else
            n(:,1:Active_Array_Size)=repmat(n_should,1,Active_Array_Size);
        end
    ...
    for p=1:length(Thickness)
        ...
        for q=1:length(Ratio_Lower2Upper)
            for w=1:length(Ratio_Upper2Reference)
                    ...
                    Current_Loop=1;
                    qqqq=1;
                    Current_Progress=100*((u-1)+((s-1)+((p-1)+((q-1)+(w-1)/length(Ratio_Upper2Reference))/length(Ratio_Lower2Upper))/length(Thickness))/length(N_Shift))/length(n_delta_array);
                    while (Current_Loop<Max_Number_of_Loop) && ((Merit_A>Merit_A_OK)||(Merit_B>Merit_B_OK)||(Merit_C>Merit_C_OK))
                        ...
                        elseif n_UR_calc==2
                            ...
                            n=n+0.1*dn_real+0.1*1i*dn_imag;
                        end
                        ...
                        if length(n_should)==1
                            ...
                            n(isinf(n))=n_should;
                        elseif n_UR_calc==1
                            n(isnan(n))=n_should(isnan(n))+n_delta_array(u);
                            n(isinf(n))=n_should(isinf(n))+n_delta_array(u);
                        elseif n_UR_calc==2
                            n(isnan(n))=n_should(isnan(n))+1i*n_delta_array(u);
                            n(isinf(n))=n_should(isinf(n))+1i*n_delta_array(u);
                        else
                        ...
                        if length(Thickness)==1 %if length(Thickness)>1 > Uniqueness test
                            %Thickness_Now=Thickness_Now+0.01*mean(dthickness);   %How if I make the thickness a array during the iteration?
        ...
        if length(N_Shift)>1 %if length(Thickness)>1 > Uniqueness test
            MSE_A_Record(s)=(sum((delta_A./Aexp).^2)/length(delta_A)).^0.5;
            MSE_B_Record(s)=(sum((delta_B./Bexp).^2)/length(delta_B)).^0.5;
            MSE_C_Record(s)=(sum((delta_C./Cexp).^2)/length(delta_C)).^0.5;
        end
    end
    
        if n_UR_calc>0
            MSE_A_Record_Wavelength(:,u)=((delta_A./Aexp).^2)/length(delta_A).^0.5;
            MSE_B_Record_Wavelength(:,u)=((delta_B./Bexp).^2)/length(delta_B).^0.5;
            MSE_C_Record_Wavelength(:,u)=((delta_C./Cexp).^2)/length(delta_C).^0.5;
        end
    end
...
if n_UR_calc == 0
    ...
    for uu=1:length(Wavelength_micron_Considered)
        MSE_Normalized_Wavelength(uu,:)=((MSE_A_Record_Wavelength(uu,:).^2+MSE_B_Record_Wavelength(uu,:).^2+MSE_C_Record_Wavelength(uu,:).^2)./min(MSE_A_Record_Wavelength(uu,:).^2+MSE_B_Record_Wavelength(uu,:).^2+MSE_C_Record_Wavelength(uu,:).^2)).^0.5;
        MSE_Unnormalized(uu,:)=((MSE_A_Record_Wavelength(uu,:).^2+MSE_B_Record_Wavelength(uu,:).^2+MSE_C_Record_Wavelength(uu,:).^2)).^0.5;
    end
    for uu=1:length(Wavelength_micron_Considered)
        Uniqueness_Range_Wavelength(uu)=abs(real(n_delta_array(find(MSE_Normalized_Wavelength(uu,:)<1.1,1,'first')))-real(n_delta_array(find(MSE_Normalized_Wavelength(uu,:)<1.1,1,'last'))));
    end
    %plot(n_delta_array,MSE_Normalized_Wavelength(71,:));
    %NNN=71;
    %plot(n_delta_array,MSE_A_Record_Wavelength(NNN,:),n_delta_array,MSE_B_Record_Wavelength(NNN,:),n_delta_array,MSE_C_Record_Wavelength(NNN,:));
    [minvalue minindex]=min(MSE_Normalized_Wavelength,[],2);
    n_delta_corrected=n_delta_array(minindex);
    dlmwrite('n_corrected.txt',real(n_should)-n_delta_corrected','delimiter','\t','newline','pc');

    plot(Wavelength_micron_Considered,Uniqueness_Range_Wavelength);
end
%xlim([4.64 4.8]);
...
dlmwrite('Uniqueness_Range_Wavelength.txt',Uniqueness_Range_Wavelength','delimiter','\t','newline','pc');
...
%Ratio_Lower2Upper=1;     %if the rear interface has larger interference eff than front interface, Ratio_Lower2Upper>1
%%
plot(Wavelength_micron_Considered,MSE_Normalized_Wavelength(:,3));

%%
plot(n_delta_array,MSE_Normalized_Wavelength(64,:));
%%
NNN=19;
plot(n_delta_array,MSE_A_Record_Wavelength(NNN,:),n_delta_array,MSE_B_Record_Wavelength(NNN,:),n_delta_array,MSE_C_Record_Wavelength(NNN,:));
xlabel('delta_k');
