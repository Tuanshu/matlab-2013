...
Thickness=(3).*1E-6;% ([-0.1:0.1:0.1]+2.55).*1E-6; In this file, it is the initial guess of thickness
n_should=1.6;
Ratio_Lower2Upper=1.21;%+[-0.05:0.005:0.05];%+[-0.2:0.02:0.2];  %([-1:0.02:1]+10.82)*1E-6;%[0.019:0.001:0.024]*1E-6;%[0.02:0.0003:0.023]*1E-6;

%%
DC_cutoff=23.5;   %works for all lateral position
Least_Separation=3;
...
mean(Thickness_Now)
Merit_A=sum((Aexp-A(n,Thickness_Now,Ratio_Lower2Upper_Now,Ratio_Upper2Reference_Now)).^2)^0.5;
Merit_B=sum((Bexp-B(n,Thickness_Now,Ratio_Lower2Upper_Now,Ratio_Upper2Reference_Now)).^2)^0.5;
Merit_C=sum((C_Temp-C(n,Thickness_Now,Ratio_Lower2Upper_Now,Ratio_Upper2Reference_Now)).^2)^0.5;
fprintf('Merit_A=%e, Merit_B=%e, Merit_C=%f\n',Merit_A,Merit_B,Merit_C);

cd('D:\Users\TuanShu\');


dlmwrite('Wavelength_micron_Considered_3micron_NewAlgorithm.txt',Wavelength_micron_Considered,'delimiter','\t','newline','pc');
dlmwrite('n_3micron_NewAlgorithm.txt',real(n_Best),'delimiter','\t','newline','pc');
dlmwrite('k_3micron_NewAlgorithm.txt',imag(n_Best),'delimiter','\t','newline','pc');
dlmwrite('T_3micron_NewAlgorithm.txt',T_Best,'delimiter','\t','newline','pc');
dlmwrite('T_exp_3micron_NewAlgorithm.txt',T_Considered,'delimiter','\t','newline','pc');