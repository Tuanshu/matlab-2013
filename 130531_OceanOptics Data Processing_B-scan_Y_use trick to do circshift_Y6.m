...
cd('D:\Users\TuanShu\130531\YPR 6\');
%% Data Loading
...
Signal_Bscan_Envelope_Temp=Signal_Bscan_Envelope;
Signal_Bscan_Envelope_Temp(find(Position_micron>55,1,'first'):end,1:800)=0;


[maxvalue maxindex]=max(Signal_Bscan_Envelope_Temp(find(Position_micron>Range_1,1,'first'):find(Position_micron>Range_2,1,'first'),:),[],1);
...
for pp=1:size(Signal_Bscan_Envelope,1)
    Signal_Bscan_Envelope_Shifted(pp,:)=conv(Signal_Bscan_Envelope_Shifted(pp,:),ones(Pixel_Average_Lateral,1)/Pixel_Average_Lateral,'same');
end
