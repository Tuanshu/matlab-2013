        ...
        for q=1:length(Ratio_Lower2Upper)
            for w=1:length(Ratio_Upper2Reference)
                    ...
                    while (Current_Loop<Number_of_Loop)
                        ...
                        Thickness_Now=Thickness_Now+0.2*mean(dthickness);   %How if I make the thickness a array during the iteration?
                        Current_Loop=Current_Loop+1;
                        %fprintf('%d/%d\n',Current_Loop,Number_of_Loop);
                        n(isnan(n))=n_should;
                        n(isinf(n))=n_should;
                        if (max(isnan(Thickness_Now)>0))+(max(isinf(Thickness_Now)>0))
