...
n_delta_array=-0.000000000001:0.0000000000001:0.000000000001;
...
if n_UR_calc == 0
    ...
    if length(Thickness)==1 %if length(Thickness)>1 > Uniqueness test
        if length(N_Shift)==1
            ...
            dlmwrite('n.txt',real(n_Best),'delimiter','\t','newline','pc', 'precision', 18);
            dlmwrite('k.txt',imag(n_Best),'delimiter','\t','newline','pc', 'precision', 18);
...
%%
plot(n_delta_array,MSE_Normalized_Wavelength(42,:));
    xlabel('delta n');
    ylabel('MSE');
%%
NNN=19;
