...
r_fp1=r(n_air,n2)+(t(n_air,n2).*t_r(n_air,n2).*r(n2,n1).*d(n2,l2))./(1-r_r(n_air,n2).*r(n2,n1).*d(n2,l2));
t_fp1=(t(n_air,n2).*t(n2,n1).*exp(1i*2*pi.*Frequency/C.*n2*l2/1E6))./(1-r_r(n_air,n2).*r(n2,n1).*d(n2,l2));

r_r_fp1=r(n1,n2)+(t(n1,n2).*t_r(n1,n2).*r(n1,n_air).*d(n1,l1))./(1-r_r(n1,n2).*r(n1,n_air).*d(n2,l2));
t_r_fp1=(t(n1,n2).*t(n2,n_air).*exp(1i*2*pi.*Frequency/C.*n2*l2/1E6))./(1-r_r(n1,n2).*r(n2,n_air).*d(n2,l2));
%%
% FP of 2nd film
r_fp2=r(n1,n3)+(t(n1,n3).*t_r(n1,n3).*r(n3,n_air).*d(n3,l3))./(1-r_r(n1,n3).*r(n3,n_air).*d(n3,l3));
t_fp2=(t(n1,n3).*t(n3,n_air).*exp(1i*2*pi.*Frequency/C.*n3*l3/1E6))./(1-r_r(n1,n3).*r(n3,n_air).*d(n3,l3));


r_r_fp2=r(n_air,n3)+(t(n_air,n3).*t_r(n_air,n3).*r(n3,n1).*d(n3,l3))./(1-r_r(n_air,n3).*r(n3,n1).*d(n3,l3));
t_r_fp2=(t(n_air,n3).*t(n3,n1).*exp(1i*2*pi.*Frequency/C.*n3*l3/1E6))./(1-r_r(n_air,n3).*r(n3,n1).*d(n3,l3));
% coupled FP of both FPs

r_total=r_fp1+(t_fp1.*t_r_fp1.*r_fp2.*d(n1,l1))./(1-r_r_fp1.*r_fp2.*d(n1,l1));
%%
...
s_sam=G.*exp(1i*4*pi*Frequency/C*l_sam/1E6).*r_total;
