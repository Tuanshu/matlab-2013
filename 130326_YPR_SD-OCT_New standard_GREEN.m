...
Ratio_Upper2Reference=1.18;
Thickness=(2.7).*1E-6;      %Initial Value
n_should=1.6;
Ratio_Lower2Upper=1.30;

%%
DC_cutoff=13;   %works for all lateral position
Least_Separation=2.5;
...
%end
Data_Temp=importdata('TEST200.txt');  
Spectrum_Sample_Frequency(:,1)=((Data_Temp(:,2)-Data_Temp(end,2)).*((Wavelength*1E-9).^2)/c);
cd(Reference_Path);
...
%    fprintf('Loading B-Scan Data...%d/%d\n',array_number,length(array));
%end
Data_Temp=importdata('TEST50.txt');  
Spectrum_Reference_Frequency(:,1)=((Data_Temp(:,2)-Data_Temp(end,2)).*((Wavelength*1E-9).^2)/c);
...
Separation_index=find(Position_micron>21,1,'first');
