...
plot(1:length(Position_micron),Signal_Bscan_Carrier(:,500));
xlabel('OPD (micron)');
ylabel('Interference Signal');

plot(Position_micron_cut,Signal_Bscan_Envelope_Cut);

[max_Signal_Bscan_Envelope_Cut maxindex_Signal_Bscan_Envelope_Cut]=max(Signal_Bscan_Envelope_Cut);
...
imagesc(10*log10(Signal_Bscan_Envelope_Cut(1:10000,:)/max(max(Signal_Bscan_Envelope_Cut))),'xdata',Lateral_Spacing:Lateral_Spacing:size(Signal_Bscan_Envelope_Cut,2)*Lateral_Spacing,'ydata',Position_micron_cut);
colormap(gray);
