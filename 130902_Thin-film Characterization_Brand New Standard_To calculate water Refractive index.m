...
Sample_Path='D:\Users\TuanShu\130830\100+\sample 2\inter.txt';
Reference_Path='D:\Users\TuanShu\130830\100+\air 2\inter.txt';
...
%%%
DC_cutoff=24;   %works for all lateral position
Least_Separation=7;
Window_Size_Right_1=3.8;  %use for the left side of 1st interface and right side of second interface
Window_Size_Left_1=3.6;  %use for the left side of 1st interface and right side of second interface
...
Aexp_oldstyle=abs(Spectrum_Sample_1(Frequency_Considered_Min_Index:Frequency_Considered_Max_Index,:))./abs(Spectrum_Reference(Frequency_Considered_Min_Index:Frequency_Considered_Max_Index,:)); 
Aexp_oldstyle=downsample(Aexp_oldstyle,Frequency_Downsample_Ratio);
...
plot(Wavelength_micron_Considered,Aexp_oldstyle);
...
if index_calculation==1   
    ...
    MSE_total_previous=999999999999;
        ...
        for q=1:length(Ratio_Lower2Upper)
            for w=1:length(Ratio_Upper2Reference)
                    ...
                    while (Current_Loop<Max_Number_of_Loop) && (MSE_total>MSE_total_OK)
                        ...
                        delta_A(isnan(delta_A))=0;
                        delta_B(isnan(delta_B))=0;
                        ...
                        if length(Thickness)==1 %if length(Thickness)>1 > Uniqueness test
                            %Thickness_Now=Thickness_Now+(dthickness);   %How if I make the thickness a array during the iteration?
                        end
                        ...
                        if max(MSE_total<MSE_total_previous)
...
subplot(3,1,1)
plot(Frequency_Considered,Aexp,Frequency_Considered,A(1.33,15E-6,0.3,0.95));



subplot(3,1,2)
plot(Frequency_Considered,Bexp,Frequency_Considered,B(1.33,15E-6,0.3,0.95));


subplot(3,1,3)
plot(Frequency_Considered,Cexp,Frequency_Considered,C(1.33,15E-6,0.3,0.95));
%%
