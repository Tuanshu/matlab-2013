...
cd('D:\Users\TuanShu\130521\Compare');

qqq=1;
xxx=1;
yyy=1;
j=3;     %14 or 15: first peak, 11 or 12: rear peak
...
plot(Position_micron,Data_New/max(abs(Data_New)),Position_micron,abs(Data_New)/max(abs(Data_New)));

Data_New_env=abs(Data_New);
Window_ave=(1:Averaging)/Averaging;
Data_New_env_ave=conv(Data_New_env,Window_ave,'same');
%for p=1:length(Data_New_env)
%    if (p+Averaging-1)<length(Data_New_env)
%        Data_New_env_ave=mean(Data_New_env(p:(p+Averaging-1)));
%    else
%        Data_New_env_ave=mean(Data_New_env(p:end));
%    end
%end
plot(Position_micron,Data_New_env_ave);
