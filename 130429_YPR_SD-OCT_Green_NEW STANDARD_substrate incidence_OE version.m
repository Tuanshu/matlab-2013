...
N_Shift=[4]';     %Physically must be integer, 若不是integer那會算錯
...
Sample_Path='D:\Users\TuanShu\TEST\111127 - 3ms 100ave 3mm v0.01 Green - 3\';
Reference_Path='D:\Users\TuanShu\TEST\111127 - 3ms 100ave 3mm v0.01 Green - 3\';
...
Thickness=[3.3:0.025:3.8]'*1E-6;      %Initial Value
n_should=1.6;
Ratio_Lower2Upper=1.25;

%%%
DC_cutoff=16;   %works for all lateral position
Least_Separation=3;
Window_Size=8;  %use for the left side of 1st interface and right side of second interface
...
cd(Sample_Path);
Data=importdata('TEST200.txt');      % Data_2: the glass data 1
...
%for array_number=1:fix(length(array)/Lateral_Downsample_Ratio)
    Data_Temp=importdata(sprintf('TEST%i.txt',200));      % Data_2: the glass data 1
...
%for array_number=1:fix(length(array)/Lateral_Downsample_Ratio)
    Data_Temp=importdata(sprintf('TEST%i.txt',50));      % Data_2: the glass data 1
...
Separation_index=find(Position_micron>21,1,'first');%round((Max_index+Max_index_Second)/2);
...
if length(Frequency_Considered)>1
    Cexp_intercept=interp1(Frequency_Considered,Cexp,0,'linear','extrap');
else
    Cexp_intercept=-175;
end
...
Cexp=Cexp-Cexp_intercept;%-C_intercept;
        ...
        for q=1:length(Ratio_Lower2Upper)
            for w=1:length(Ratio_Upper2Reference)
                    ...
                    while (Current_Loop<Max_Number_of_Loop) && ((Merit_A>Merit_A_OK)||(Merit_B>Merit_B_OK)||(Merit_C>Merit_C_OK))
                        ...
                        C_Temp=(C(n,Thickness_Now,Ratio_Lower2Upper_Now,Ratio_Upper2Reference_Now))+N_Shift(s);
