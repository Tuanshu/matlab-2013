...
d_Position=motor_speed/frame_rate;
Binning=2;
Lateral_Spacing=4.75/(759-256)*2*Binning;

SPF=50;
LPF=100;
%% Height generation
cd('D:\Users\TuanShu\131217\');
MOVIE=aviread('test.avi');
%%
imagesc(MOVIE(191).cdata);

image_index=70:300;
%%
data(1:length(image_index),1:size(MOVIE(231).cdata,1)/Binning,1:size(MOVIE(231).cdata,2)/2/Binning)=0;
for p=1:length(image_index)
    Image_Temp=MOVIE(image_index(p)).cdata;
    
    [m,n]=size(Image_Temp); %M is the original matrix

    Image_Temp=sum( reshape(Image_Temp,1,[]) ,1 );
    Image_Temp=reshape(Image_Temp,m/1,[]).';

    Image_Temp=sum( reshape(Image_Temp,2,[]) ,1);
    Image_Temp=reshape(Image_Temp,n/2,[]).';
...
plot(image_index,data(:,120,100),image_index,data_envelope(:,120,100));
...
Surface_profile_New=conv2(Surface_profile,filter,'same');

%%
Reducing_factor=1;
clear X_temp_reduced Y_temp_reduced
...
X_temp_reduced(1:round(size(Surface_profile,1)/Reducing_factor),1:round(size(Surface_profile,2)/Reducing_factor))=0;
Y_temp_reduced(1:round(size(Surface_profile,1)/Reducing_factor),1:round(size(Surface_profile,2)/Reducing_factor))=0;

for p=1:size(X_temp_reduced,2)
    X_temp_reduced(:,p)=p*Reducing_factor.*Lateral_Spacing;
end
for q=1:size(Y_temp_reduced,1)
    Y_temp_reduced(q,:)=q*Reducing_factor.*Lateral_Spacing;
end


Surface_profile_reduced=interp2(X_temp,Y_temp,Surface_profile,X_temp_reduced,Y_temp_reduced,'cubic');
Surface_profile_New=interp2(X_temp_reduced,Y_temp_reduced,Surface_profile_reduced,X_temp,Y_temp,'linear');
imagesc(Surface_profile_reduced);
%%
plot(1:size(Surface_profile,2),Surface_profile(100,:),1:size(Surface_profile,2),Surface_profile_New(100,:));
plot(Surface_profile_reduced(round(100/Reducing_factor),:));

Z=Surface_profile_New.*d_Position.*mask;
Z(isnan(Z))=0;
