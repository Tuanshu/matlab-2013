        ...
        for q=1:length(Ratio_Lower2Upper)
            for w=1:length(Ratio_Upper2Reference)
                    ...
                    while (Current_Loop<Max_Number_of_Loop)% && ((Merit_A>Merit_A_OK)||(Merit_B>Merit_B_OK)||(Merit_C>Merit_C_OK))
                        ...
                        %C_Temp_add_dn=C(n+dn,Thickness_Now,Ratio_Lower2Upper_Now,Ratio_Upper2Reference_Now);
                        %C_Temp_add_jdn=C(n+1i*dn,Thickness_Now,Ratio_Lower2Upper_Now,Ratio_Upper2Reference_Now);
                        %C_Temp_add_dn=diff(C_Temp_add_dn,[],1);
                        %C_Temp_add_jdn=diff(C_Temp_add_jdn,[],1);
                        ...
                        if size(C_Temp,1)>1
                            ...
                            dC_dn_real(size(dC_dn_real,1)+1,:)=dC_dn_real(size(dC_dn_real,1),:);
                            dC_dn_imag(size(dC_dn_imag,1)+1,:)=dC_dn_imag(size(dC_dn_imag,1),:);
                            dC_dthickness(size(dC_dthickness,1)+1,:)=dC_dthickness(size(dC_dthickness,1),:);
                        end
