...
cd('D:\Users\TuanShu\121106_Eye1\');
...
plot((1:(size(Image_Stack_New,1)))*Frame_Axial_Spacing,real(Image_Stack_New(:,240,360)));
xlabel('Axial Position (Micron)');
ylabel('Amplitude (a.u.)');

Height2=Height;
Height2((Xgrid-Starting_X).^2+(Ygrid-Starting_Y).^2>R_Weight^2)=0;
plot(0:Lateral_Spacing:Lateral_Spacing*(size(Height,2)-1),Height2(400,:),0:Lateral_Spacing:Lateral_Spacing*(size(Height,2)-1),CheckCurve(400,:));
xlabel('(Micron)');
ylabel('(Micron)');
