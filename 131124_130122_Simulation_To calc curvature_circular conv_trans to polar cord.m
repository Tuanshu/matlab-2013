...
conic=0.7;
R_given=7800;
R_given_2=7900;     %% 注意!! 此處程式有一個bug (應該是跟MATLAB的eig function定義有關), 若把兩者倒過來放, 會得到錯誤結果
                    % 說不定跟之後Z方向必須要乘一個-1是相同的理由
                    % 注意, 在把上述-1乘上去後, 看起來就不像bow-tie了, 可能還是得加上像差影響
...
Z=((R_given*R_given)^0.5).*(1-((X-X_Center)./R_given).^2-((Y-Y_Center)./R_given_2).^2).^0.5;
Z(abs(imag(Z))>0)=0;
