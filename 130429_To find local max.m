...
Data=importdata('130429_keratinocyte layers.txt');

Position=Data(:,1);

Signal=Data(:,2:end);

for p=1:size(Signal,2)
    Left_Limit=find(Signal(:,p)>Threshold,1,'first');
    Right_Limit=find(Signal(:,p)>Threshold,1,'last');
    Signal(1:Left_Limit,p)=0;
    Signal(Right_Limit:end,p)=0;
end
%%diff是後面減前面, check
