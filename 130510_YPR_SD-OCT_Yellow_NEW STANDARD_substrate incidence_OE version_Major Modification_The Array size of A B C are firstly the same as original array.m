...
N_Shift=[-45]';     %Physically must be integer, 若不是integer那會算錯
...
Sample_Path='D:\Users\TuanShu\130509\130509_sub_Y_sam_2.txt';
Reference_Path='D:\Users\TuanShu\130509\130509_sub_Y_glass.txt';
...
Ratio_Upper2Reference=1.25;
Thickness=[4.75]'*1E-6;      %Initial Value
n_should=1.6;
Ratio_Lower2Upper=1.27;

%%%
DC_cutoff=20;   %works for all lateral position
Least_Separation=2;
...
Window_Size_Right_2=4;  %use for the left side of 1st interface and right side of second interface
...
Frequency_Array_Enlength_factor=8;
N=8192*Frequency_Array_Enlength_factor;
...
Max_Frequency=c/(Min_Wavelength*1E-9)*Frequency_Array_Enlength_factor;             %Hz
...
if length(array)>1
    ...
    Data_Temp=importdata(Reference_Path);      % Data_2: the glass data 1
...
Spectrum_Reference=Spectrum_Reference(1:N,:);
...
Signal_Sample_1(1:N,1:length(array))=0;
Signal_Sample_2(1:N,1:length(array))=0;
...
Spectrum_Sample_1=2*Spectrum_Sample_1(1:N,:);                

Spectrum_Sample_2=ifft(Signal_Sample_2,[],1);
Spectrum_Sample_2=2*Spectrum_Sample_2(1:N,:);


Spectrum_Reference=ifft(Signal_Reference,[],1);
Spectrum_Reference=2*Spectrum_Reference(1:N,:);                
...
subplot(3,1,3)
plot(Frequency_Considered,Cexp,Frequency_Considered,C(n,Thickness_Now,Ratio_Lower2Upper_Now,Ratio_Upper2Reference_Now));
